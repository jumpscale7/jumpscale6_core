...
class InstallTools():
    ...
    def getGitRepo(self,url,dest=None,login=None,passwd=None,depth=None,reset=False):
        """
        ...
        if self.exists(checkdir):
            ...
            cmd="cd %s;git clone %s %s"%(dest,url,dest)
            if depth<>None:
                cmd+=" --depth %s"%depth        
        self.executeInteractive(cmd)

    def getGitReposListLocal(self):
        repos={}
        for top in self.listDirsInDir("/opt/code/", recursive=False, dirNameOnly=True, findDirectorySymlinks=True):
            for account in self.listDirsInDir("/opt/code/%s"%top, recursive=False, dirNameOnly=True, findDirectorySymlinks=True):
                accountdir="/opt/code/%s/%s"%(top,account)
                if self.exists(path="%s/.git"%accountdir):
                    #is for when repo is one level higher as usual        
                    repos[account]=accountdir
                else:
                    for reponame in self.listDirsInDir("/opt/code/%s/%s"%(top,account), recursive=False, dirNameOnly=True, findDirectorySymlinks=True):
    ...
    def pushGitRepo(self,message,name="",path="",update=True):
        """
        ...
        if name<>"": 
            result=[]       
            repos=self.getGitReposListLocal()
            if not repos.has_key(name):
                raise RuntimeError("Dit not find repo for:'%s'"%name)
            path=repos[name]
