...
import sys
# sys.path.append(j.system.fs.joinPaths(j.dirs.getLibPath(),"baselib","git"))
# import hglib
...
class GitClient:    
    ...
    def __init__(self, gitbasedir, remoteUrl="", branchname=None, cleandir=None):
        """
        ...
        if remoteUrl<>"":
            self._log("git remoteurl:%s"%(remoteUrl),category="config")
        
        if (not isinstance(gitbasedir, basestring) or not isinstance(remoteUrl, basestring))\
         or (branchname and not isinstance(branchname, basestring)):
            raise ValueError("Input to gitclient need to be all strings")
        ...
        #     C+="\n[hostfingerprints]\ngitlab.org =24:9c:45:8b:9c:aa:ba:55:4e:01:6d:58:ff:e4:28:7d:2a:14:ae:3b\n"
...
"""
        ...
        if j.system.fs.exists(self.basedir) and not j.system.fs.exists(ignorefilepath):
            ...
            #add items not in gitignore yet
    ...
    def verifyfix(self):
        ...
        """
        self._log("git verify %s" % (self.basedir),2)
        if not self.verify(False):
            msg="git directory on %s is corrupt." % (self.basedir)
            if j.application.shellconfig.interactive:
                j.console.echo(msg)
                ...
                self._raise("git directory on %s is corrupt." % (self.basedir))
        self.isConnected()
    ...
    def _log(self, message, level=5,category=""):
        ...
        j.clients.git.log(message,category,level)

    def _raise(self, message):        
        message="ERROR gitclient: %s\nPlease fix the merurial local repo manually and restart failed git action.\nRepo is %s" % (message, self.basedir)
    ...
    def getModifiedFiles(self):
        ...
        remarks
        - missing means, file referenced in git local repo but no longer on filesystem (! in hg status) 
        - notracked mans, file is on filesystem but not in repo (?)
        - removed means, git repo knows file has been removed from filesystem (R)
    ...
    def checkbranch(self):
        ...
        if self.branchname:
            if self.getbranchname() != self.branchname:
                force=False
                if j.application.shellconfig.interactive:
                    if j.gui.dialog.askYesNo("\nBranchnames conflict for repo, jshell git client has branchname: %s and branchname on filesystem: %s\nThis might result in loosing code changes. \nDo you want to continue, this will pull & update to the branch?" % (self.branchname,self.getbranchname())):
                        self.pullupdate(force=True)
                if not force:
                    self._raise("Branchnames conflict for repo, jshell git client has branchname: %s and branchname on filesystem: %s" % (self.branchname,self.getbranchname()))
    ...
    def reset(self):
        ...
        git saves some state linked to the repo object. If a hg pull
