...
class KeyValueStoreFactory(object):
    ...
    def getRedisStore(self, namespace='',host='localhost',port=7768,db=0,password='',masterdb=None,changelog=True):
        '''
        Gets a memory key value store.
        ...
        if key not in self._cache:
            self._cache[key] = RedisKeyValueStore(namespace=namespace,host=host,port=port,db=db,password=password,serializers=[],masterdb=masterdb, changelog=changelog)
        return self._cache[key]
