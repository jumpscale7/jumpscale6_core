...
class ZDaemonFactory():
    ...
    def getZDaemon(self, port=4444, name="", nrCmdGreenlets=50, sslorg="", ssluser="", sslkeyvaluestor=None):
        ...
        zd=j.core.zdaemon.getZDaemon(port=6555,nrCmdGreenlets=50)
    ...
    def getZDaemonClient(self, addr="127.0.0.1", port=6555, org="myorg", user="root", passwd="1234", ssl=False, category="core" ):
        """
        example usage, see example for server at self.getZDaemon

        client=j.core.zdaemon.getZDaemonClient(ipaddr="127.0.0.1",port=6555,login="root",passwd="1234",ssl=False)
    ...
    def getZDaemonAgent(self, ipaddr="127.0.0.1", port=6555, org="myorg", user="root", passwd="1234", ssl=False, reset=False, roles=[]):
        """
        example usage, see example for server at self.getZDaemon

        agent=j.core.zdaemon.getZDaemonAgent(ipaddr="127.0.0.1",port=6555,login="root",passwd="1234",ssl=False,roles=["*"])
        agent.start()
