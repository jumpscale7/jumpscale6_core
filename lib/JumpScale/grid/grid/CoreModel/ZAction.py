...
class ZAction(ZBase):
    ...
    def __init__(self, ddict={}, name="", code="", md5="", path="", category="", errordescr="", recoverydescr="", maxtime=0):
        if ddict <> {}:
            ...
            self.code = code
            if md5 == "":
                self.codemd5 = j.tools.hash.md5_string(self.code)
            else:
                self.codemd5 = md5
            self.path = path
    ...
    def getUniqueKey(self):
        ...
        """
        return j.tools.hash.md5_string(str(self.gid) + str(self.bid) + str(self.name) + self.codemd5 + self.path + self.category + self.errordescr + self.recoverydescr + str(self.maxtime))
