...
class CodeGenerator:
    def __init__(self):
        self.codepath=o.system.fs.joinPaths(o.dirs.varDir,"code")
        ...
        o.system.fs.writeFile(o.system.fs.joinPaths(self.codepath,"__init__.py"),"")
    ...
    def getClassActorLocal(self,appname,actor,typecheck=True,dieInGenCode=True):
        ...
        spec=o.core.specparser.getActorSpec(appname,actor)
        #spec=o.core.specparser.findSpec(appname=appname,actorname=actor,specname=actor,type=spectype,findOnlyOne=True)
    ...
    def getClassActorRemote(self,appname,actor,typecheck=True,dieInGenCode=True,instance=0,redis=False,wsclient=None,codepath=None):
        ...
        spec=o.core.specparser.getActorSpec(appname,actor)
        #spec=o.core.specparser.findSpec(appname=appname,actorname=actor,specname=actor,type=spectype,findOnlyOne=True)
        classs=self.generate(spec,type=type,typecheck=typecheck,dieInGenCode=dieInGenCode,instance=instance,redis=redis,wsclient=wsclient,codepath=codepath)
        return classs
    ...
    def getClassEnumeration(self,appname,actor,enumname,typecheck=True,dieInGenCode=True):
        ...
        #if self.classes.has_key(key):
            #return self.classes[key]
        spec=o.core.specparser.findSpec(appname=appname,actorname=actor,specname=enumname,type=spectype,findOnlyOne=True)
    ...
    def generate(self,spec,type,typecheck=True,dieInGenCode=True,appserverclient=None,instance=0,redis=False,wsclient=None,\
        ...
        #     cg=CodeGeneratorWhoosh(spec,typecheck,dieInGenCode)
            ...
            self.classes[name]="o.enumerators.%s"%cg.getClassName()
        ...
        if classpath<>None and o.system.fs.exists(path):
            pass
        else:
            o.system.fs.writeFile(path,code)

        if makeCopy:
            o.system.fs.writeFile(path.replace(".py",".gen.py"),code)
    ...
    def _getCodeLocation(self,type,appname,spectype,specactor,specname):
        ...
        path=o.system.fs.joinPaths(self.codepath,name+".py")
        return name,path

    def _import(self,name,classname,codepath):
        pp=o.system.fs.getDirName(o.system.fs.pathNormalize(codepath))
        # curpath=o.system.fs.getcwd()
        # o.system.fs.changeDir(pp)
        ...
        try:
            ...
            o.errorconditionhandler.raiseBug("","spec.import",e)
