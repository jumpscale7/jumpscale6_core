...
class CodeGeneratorActorRemote(CodeGeneratorBase):
    def __init__(self,spec,typecheck=True,dieInGenCode=True,instance=0,redis=False,wsclient=None,codepath=None):
        ...
        self.actorpath=o.system.fs.joinPaths(codepath,spec.appname,spec.actorname)
        ...
        if wsclient<>None:
            ...
            self.ip,self.port,self.secret=o.core.appserver6.gridmap.get(spec.appname,spec.actorname,instance)
...
if resultcode<>0:
    ...
    if o.basetype.dictionary.check(result) and result.has_key("result"):
