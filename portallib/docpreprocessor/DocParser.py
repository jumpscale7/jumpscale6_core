...
class DocParser():
    def parseDoc(self,doc):
        ...
        if doc.title=="":
            doc.title=o.system.fs.getBaseName(doc.path).replace(".wiki","")
        ...
        if doc.name=="":
            doc.name=o.system.fs.getBaseName(doc.shortpath).replace(".wiki","").lower().strip()
        if doc.pagename=="":
    ...
    def _findItem(self,text,item="@owner",maxitems=1,removecolon=True):
        ...
        def process(arg,line):
            ...
            result.append(line)
            return False
        text2=o.codetools.regex.replaceLines( process, arg="", text=text, includes=["%s.*"%item], excludes='')
        ...
        for item in text.strip().split(" "):
            ...
            if state=="start":
                if item[0] in keys:
                    ...
                    #found user or group
                    if not o.codetools.regex.match("\w*:\w*",item):
                        descr+=item+" "
                        state="endofmeta"
                    elif user=="":
                        splitted=item.split(":")
    ...
    def addUniqueId(self,line,fullPath,ttype="sprint"):
        ...
        if id1==0:
            ...
            id1=o.base.idpreprocessor.generateIncrID("%sid"%ttype,self.service)
