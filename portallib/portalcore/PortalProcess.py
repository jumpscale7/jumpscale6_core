...
class PortalProcess():
    ...
    def __init__(self,processNr=1,mainLoop=None,inprocess=False,cfgdir="",startdir=""):
        ...
        # o.errorconditionhandler.setExceptHook() #@does not do much?
        ...
        if o.system.fs.isLink(startdir,True):
            #we are link do not use this config info
            name=o.system.fs.getDirName(startdir+"/",True)+"_localconfig"
            newpath=o.system.fs.joinPaths(o.system.fs.getParent(startdir+"/"),name)
            if not o.system.fs.exists(newpath):
                o.system.fs.createDir(newpath)
                pathcfgold=o.system.fs.joinPaths(startdir,"cfg")                
                o.system.fs.copyDirTree(pathcfgold,newpath)                                
            self.cfgdir=newpath

        ini=o.tools.inifile.open(self.cfgdir+"/appserver.cfg")
        ...
        #self.specpath=ini.getValue("main","specpath")
            ...
            #self.specpath="specs"
        #if not o.system.fs.exists(self.specpath):
            #raise RuntimeError("spec path does have to exist: %s" % self.specpath)

        dbtype=ini.getValue("main","dbtype").lower().strip()
        if dbtype=="fs":
            self.dbtype = o.enumerators.KeyValueStoreType.FILE_SYSTEM
        elif dbtype=="mem":
            self.dbtype = o.enumerators.KeyValueStoreType.MEMORY
        elif dbtype=="redis":
            self.dbtype = o.enumerators.KeyValueStoreType.REDIS
        elif dbtype=="arakoon":
            self.dbtype = o.enumerators.KeyValueStoreType.ARAKOON
        else:
        ...
        if self.wsport>0 and inprocess==False:
            self.webserver = o.web.geventws.get(self.wsport,self.secret,wwwroot=ini.getValue("main","wwwroot"),\
        ...
        if self.ismaster:
            ...
            self.master=o.apps.system.master.extensions.master
            #self.master._init()
    ...
    def startNginxServer(self):
        ...
        if local:
            if o.system.platformtype.isWindows():
                ...
                cfgpath=o.system.fs.joinPaths(apppath,"conf","sites-enabled","appserver.conf")
                o.system.fs.writeFile(cfgpath,configtemplate)

                apppath2=o.system.fs.joinPaths(apppath,"start.bat")
                ...
                pid= o.system.windows.getPidOfProcess("nginx.exe")
                if pid<>None:
                    o.system.process.kill(pid)

                pid= o.system.windows.getPidOfProcess("php-cgi.exe")
                if pid<>None:
                    o.system.process.kill(pid)
                ...
                cfgpath=o.system.fs.joinPaths("/etc/nginx/sites-enabled","appserver.conf")
                o.system.fs.writeFile(cfgpath,configtemplate)
                ...
                if not o.system.fs.exists("/etc/nginx/nginx.conf.backup"):
                    ...
                    o.system.fs.writeFile("/etc/nginx/nginx.conf",configtemplate2)
                    o.system.process.execute("/etc/init.d/nginx restart")

                o.system.process.execute("/etc/init.d/nginx reload")
                ...
                o.system.process.executeAsync("php-cgi",["-b", "127.0.0.1:9000","-c","/opt/qbase6/apps/acloudEventHandler/libphp/php.ini"],False,False,False,False,False)
    ...
    def startConnectRedisServer(self,appname,actorname,instance=0):
        ...
        if allinone==False:
            ...
            if rediscfg<>False:
                ...
                launch=False
                if o.clients.redis.ping(ip,port,secret)==False:
        ...
        if launch:
            ...
            if o.system.platformtype.isWindows():
                ...
                redisapppath=""
                redisdbdpath=o.system.fs.joinPaths(o.dirs.varDir,"redis")

            o.system.fs.createDir(redisdbdpath)
            ...
            cfgpath=o.system.fs.joinPaths(redisdbdpath,"redis_%s_%s.conf"%(appname,actorname))
            o.system.fs.writeFile(cfgpath,configtemplate)

            if o.system.platformtype.isWindows():
                apppath2=o.system.fs.joinPaths(redisapppath,"start.bat")
                cmd="%s %s %s" % (apppath2,redisapppath,cfgpath)
                print "start redis for actor %s %s, cmd was %s" % (appname,actorname,cmd)
                o.system.process.executeAsync(cmd,outputToStdout=False)
            else:
                o.system.process.executeAsync("redis-server",[cfgpath],False,False,False,False,False)
    ...
    def processErrorConditionObject(self,eco):
        print eco
        eco2=o.apps.system.errorconditionhandler.model_errorcondition_new()
        ...
        eco2=o.apps.system.errorconditionhandler.processECO(eco2)
        
    def redirectErrors(self):
        o.errorconditionhandler.processErrorConditionObject=self.processErrorConditionObject #@todo kds redirect

    def restartInProcess(self, app):
        args = sys.argv[:]
        args.insert(0, sys.executable)
        apppath = o.system.fs.joinPaths(o.dirs.appDir, app)
        max_fd = 1024
