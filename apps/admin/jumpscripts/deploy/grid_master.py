...
def action(node):
    ...
    c.run("jpackage install -n processmanager -r --debug")
    c.run("jsprocess start -n processmanager")
    c.run("jpackage install -n portal -r --debug")
    c.run("jpackage install -n workers -r --debug")
    c.run("jpackage install -n grid_portal")
    c.run("jsprocess start -c")
