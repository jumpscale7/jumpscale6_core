...
class FilesystemReal(FilesystemBase):
    ...
    def mkdir(self, path):
        ...
        if id != "":
            if ttype == "spaces":
                ...
                j.apps.system.contentmanager.notifySpaceNewDir(id, None, path)
    ...
    def rmdir(self, path):
        ...
        if not j.system.fs.exists(path2):
            return
        j.system.fs.removeDirTree(path2)
    ...
    def identify(self, path):
        ...
        items = j.system.fs.pathNormalize(path).split(os.sep)
        type = ""
        id = 0
        key = ""
        ...
        while len(items) > 0:
            ...
            id = j.system.fs.getParentDirName(curpath + "/")
            if j.system.fs.exists(j.system.fs.joinPaths(curpath, ".space")):
                # found space
                type = "spaces"
                break
            elif j.system.fs.exists(j.system.fs.joinPaths(curpath, ".bucket")):
                # found space
                type = "buckets"
                break
            elif j.system.fs.exists(j.system.fs.joinPaths(curpath, ".actor")):
                # found space
                type = "actors"
                break
            items.pop()
