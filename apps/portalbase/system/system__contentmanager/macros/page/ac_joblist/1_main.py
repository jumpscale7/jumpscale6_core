...
def main(j, args, params, tags, tasklet):
    ...
    header = ["ID", "Roles", "JSName", "JSOrganization", "args", "Timeout", "SessionID", "Children", "Status"]
    rows = []
    ...
    #     polled = datetime.datetime.fromtimestamp(session["lastpoll"]).strftime('%Y-%m-%d %H:%M:%S')
        rows.append(["%s__/Docs/ACJob?jobid=%s" % (job['id'], job['id']), job['roles'], job["jsname"], job["jsorganization"], job["args"], job["timeout"], job['sessionid'], job['children'], status])
