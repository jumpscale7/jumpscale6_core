...
class MessageHandler:
    def __init__(self):
        """
        for doc see messagehandler part in doc section of openwizzy6 repo
        """
        ...
        if len(o.application.whoAmIBytestr)<>6:
            raise RuntimeError("Cannot start messagehandler, the whoAmiBytestr on o.application should be 6 bytes")
    ...
    def data2Message(self,ttype,data):
        ...
        """
        #print len(data)
        #print "who:%s" % len(o.application.whoAmIBytestr)
        ...
        msg= struct.pack("I",dataLength+25)+struct.pack("I",0)+\
            o.application.whoAmIBytestr+ttype+struct.pack("i",crc)+data
    ...
    def redirectStdOutStdErrorToLogger(self,yesno):
        if yesno:
            ...
            from OpenWizzy.core.logging.RedirectStreams import redirectStreams
    ...
    def queueMessage(self,message):
        path=o.system.fs.joinPaths(o.dirs.varDir,"messagequeue",o.application.appname)
        o.system.fs.createDir(path)
        path="%s/%s.queue" % (path,int(round(o.base.time.getTimeEpoch()/300,0)*300))
        o.system.fs.writeFile(path,message,True) #will append to file
    ...
    def processQueue(self,method,messageType=0,applicationName=None,removeWhenDone=False):
        ...
        if self.ping():
            if applicationName==None:
                applicationName=o.application.appname
            path=o.system.fs.joinPaths(o.dirs.varDir,"messagequeue",applicationName)
            if o.system.fs.exists(path):
                for item in o.system.fs.listFilesInDir(path):
                    contentOut=""
                    ...
                    if removeWhenDone:
                        if contentOut=="":
                            ...
                            o.system.fs.remove(path)
                        else:
                            o.system.fs.writeFile(path,contentOut)
    ...
    def sendMessage(self,message,retries=3,ignoreError=False,queue=True):
        ...
        def echo2(msg):
            if self._silentRetry==False:
                o.console.echo( msg,log=False)

        if self._client==None:  #if none means logserver is not available
            if self.start+60<o.base.time.getTimeEpoch() or ignoreError==False:
    ...
    def _monkeyPatch(self):
        o.logger.cleanup=self._notImplemented
        o.logger.logTargets=[]
        o.logger.cleanupLogsOnFilesystem=self._notImplemented
        #o.logger.consoleloglevel
        o.logger.clear=self._notImplementedIgnore
        o.logger.decodeLogMessage=self._notImplemented
        #o.logger.maxlevel
        o.logger.close=self._notImplementedIgnore
        o.logger.exception=self._notImplemented
        o.logger.logTargetAdd=self._notImplemented
        o.logger.nolog=False
        o.logger.log=self.log
    ...
    def log(self, message, level=5, tags="",dontprint=False,category=""):
        ...
        try:
            if level < (o.logger.consoleloglevel + 1) and not dontprint and o.application.shellconfig.interactive:
                o.console.echo(message,log=False)
            if o.logger.nolog:
                return
            if level<o.logger.maxlevel+1:
                #send to logserver
