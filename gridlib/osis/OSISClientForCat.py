...
class OSISClientForCat():
    ...
    def delete(self,namespace,category,key):
        return self.client.sendcmd("delete",namespaceid=self.namespaceid,catid=self.catid,key=key)

    def destroy(self,namespace,category):
        return self.client.sendcmd("destroy",namespaceid=self.namespaceid,catid=self.catid)

    def list(self,namespace,category,prefix=""):
        return self.client.sendcmd("list",namespaceid=self.namespaceid,catid=self.catid,prefix=prefix)

    def search(self,namespace,category,query, start=0, size=10):
        return self.client.sendcmd("search",namespaceid=self.namespaceid,catid=self.catid,query=query,
