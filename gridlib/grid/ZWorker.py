...
class ZWorker(GeventLoop):
    ...
    def init(self):
        ...
        self.identity="%s_%s_%s_%s"%(o.core.grid.processobject.gid,o.core.grid.processobject.bid,o.core.grid.processobject.nid,self.instance)

        self.client.registerWorker(obj=o.core.grid.processobject,roles=self.roles,instance=self.instance,identity=self.identity)

        o.logger.log("worker:%s, roles:%s"%(self.identity,self.roles) , level=3, category="zworker.init")
    ...
    def initWorkerSocket(self):
        ...
        # self.identity = "%s_%s_%s"%(o.core.grid.nid,self.instance,self.workerid)
        ...
        o.logger.log("zmq socket ready, identity:%s"%self.identity,level=5,category="zworker.init")
    ...
    def process(self,jobobj):
        if self.actions.has_key(jobobj.actionid):
            ...
            action=o.core.grid.zobjects.getZActionObject(action)
    ...
    def reset(self):
        ...
        #print reset worker
        o.errorconditionhandler.raiseOperationalWarning(msgpub="reset zworker",category="zbroker")
        ...
        while True:
            ...
            if socks.get(self.worker) == zmq.POLLIN:
                ...
                if len(frames) > 1:
                    ...
                    o.errorconditionhandler.raiseOperationalWarning(msgpub="error in message send to zworker",message=msg,category="zbroker")
                self.interval = self.INTERVAL_INIT
            else:
                self.liveness -= 1
                if self.liveness == 0:
                    o.logger.log("ZWorker heartbeat failure or could not connect to broker, reconnecting in %0.2fs" % self.interval, level=3, category="zworker.connection")
