...
class FilesystemReal(FilesystemBase):
    ...
    def chdir(self, ftppath):
        ...
        if not q.system.fs.exists(path):
            ...
            if self.checkDirIsInJustDeleted():
    ...
    def mkdir(self, path):
        ...
        if id<>"":
            if ttype=="spaces":
                ...
                q.apps.system.contentmanager.notifySpaceNewDir(id,None,path)
    ...
    def rmdir(self, path):
        ...
        if not q.system.fs.exists(path2):
            return
        q.system.fs.removeDirTree(path2)
    ...
    def identify(self,path):
        ...
        path=self.ftp2fs(path)
        path=q.system.fs.getDirName(path)
        items=q.system.fs.pathNormalize(path).split(os.sep)
        type=""
        id=0
        key=""
        ...
        while len(items)>0:
            ...
            id=q.system.fs.getParentDirName(curpath+"/")
            if q.system.fs.exists(q.system.fs.joinPaths(curpath,".space")):
                #found space
                type="spaces"
                break
            elif q.system.fs.exists(q.system.fs.joinPaths(curpath,".bucket")):
                #found space
                type="buckets"
                break
            elif q.system.fs.exists(q.system.fs.joinPaths(curpath,".actor")):
                #found space
                type="actors"
                break
            items.pop()
